...
class FileSelectorApp(tk.Tk):
    def __init__(self, initial_dir=None):
        super().__init__()
        self.title("Multi File Selector [Word to PDF]")
        ...
        #self.merge_button = ttk.Button(self.main_frame, text="Convert to PDF and Merge", command=self.convert_and_merge)
    ...
    def convert_and_merge(self):
        ...
        if selected_files:
            try:
                ...
                output_pdf = filedialog.asksaveasfilename(initialfile=output_name, defaultextension=".pdf", filetypes=[("PDF files", "*.pdf")])
                if output_pdf:
