...
class FileSelectorApp(tk.Tk):
    def __init__(self, initial_dir=None):
        super().__init__()
        self.title("Multi File Selector [Excel to PDF]")
        ...
        self.convert_sheets_button = ttk.Button(self.main_frame, text="Convert Sheets to PDF", command=self.convert_sheets_to_pdf)
        self.convert_sheets_button.pack(pady=5)
        ...
        if selected_files:
            ...
            for file in selected_files:
                try:
                    ...
                    messagebox.showerror("Conversion Error", f"Error converting sheets in {file}: {e}")
            excel.Quit()
            os.startfile(new_dir)
            messagebox.showinfo("Conversion Complete", "Selected sheets have been converted to individual PDFs.")
        else:
            messagebox.showwarning("No Selection", "No files selected for conversion")
