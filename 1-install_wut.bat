...
if not exist "%MINICONDA_PATH%" (
    ...
    echo "%MINICONDA_INSTALLER%" /InstallationType=JustMe /RegisterPython=1 /AddToPath=0 /S /D=%MINICONDA_PATH%
    "%MINICONDA_INSTALLER%" /InstallationType=JustMe /RegisterPython=1 /AddToPath=0 /S /D=%MINICONDA_PATH%

)

REM Minicondaのインストール後に環境変数を再読み込み
call "%MINICONDA_PATH%\Scripts\conda.exe" init
